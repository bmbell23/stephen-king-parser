...
class KingWorksParser:
    ...
    def __init__(self):
        self.request_manager = RequestManager(rate_limit=0.5)  # Decreased from 1.0 to 0.5 seconds
        self.works_dict = {}
    ...
    def extract_available_formats(self, link: str) -> str:
        ...
        try:
            ...
            format_sections = soup.find_all(['div', 'section'], class_=lambda x: x and 'format' in x.lower())
            ...
            if len(formats) < len(format_indicators):
                ...
                for container in containers:
                    ...
                    for format_type, indicators in format_indicators.items():
                        if format_type not in formats and any(indicator in text for indicator in indicators):
                            formats.add(format_type)
                            logger.info(f"Found format {format_type} in general content")
            ...
            if meta_description:
                ...
                for format_type, indicators in format_indicators.items():
                    if format_type not in formats and any(indicator in desc_text for indicator in indicators):
                        formats.add(format_type)
    ...
    def batch_process_works(self, works: List[BeautifulSoup], batch_size: int = 20) -> List[WorkData]:
    ...
    def format_row_for_export(self, work_data: WorkData) -> List[str]:
        ...
        available_in = (
            self.create_excel_hyperlink(work_data.available_in_link, work_data.available_in)
    ...
    def generate_html_table(self, rows: List[List[str]]) -> str:
        ...
        table_html = [
            ...
            '<th class="narrow-col">HC</th>',
            '<th class="narrow-col">PB</th>',
            '<th class="narrow-col">EBOOK</th>',
            '<th class="narrow-col">AUDIO</th>',
            '<th class="narrow-col">MOVIE</th>',
            '<th class="narrow-col">TV</th>',
            '</tr>',
            '</thead>',
            '<tbody>'
        ]
        ...
        for row in rows:
            ...
            collection_html = self.excel_hyperlink_to_html(collection) if collection else ''
            ...
            # Add the format columns
            table_html.extend([f'<td class="narrow-col">{cell}</td>' for cell in row[6:]])

            table_html.append('</tr>')

        table_html.extend(['</tbody>', '</table>'])

        # Add the CSS directly before the table
        css = """
        <style>
        /* Table structure and appearance */
        .works-table {
            width: 100% !important;
        ...
        .works-table a:hover {
            ...
            color: var(--blood-red-hover) !important;
        }
        ...
        .works-table th.narrow-col,
        .works-table th.date-col,
        .works-table th.type-col,
        table.dataTable thead th.narrow-col,
        table.dataTable thead th.date-col,
        table.dataTable thead th.type-col {
            font-family: 'Portrait Condensed', sans-serif !important;
        ...
        .works-table th.title-col,
        .works-table th.collection-col,
        table.dataTable thead th.title-col,
        table.dataTable thead th.collection-col {
            font-family: 'Portrait Condensed', sans-serif !important;
    ...
    def export_to_html(self, filename: str, works_data: List[List[str]]):
        ...
        works_data = [[str(cell) if cell is not None else '' for cell in row] for row in works_data]
...
<head>
    ...
    <style>
        :root {
            --blood-red: #8B0000;
            --blood-red-hover: #A52A2A;
            --background-color: #1a1c1e;    /* Lightened from #141618 */
            --table-background: #22252a;    /* Lightened from #1c1f23 */
            --hover-color: #2d3137;         /* Lightened from #262a2f */
        ...
        /* Make Titles and Collections bold, lighter red, and bigger */
        table.works-table > tbody > tr > td:nth-child(4), /* Title column */
        table.works-table > tbody > tr > td:nth-child(6)  /* Available In (Collections) column */ {
            font-weight: 700 !important;
            font-family: 'Plus Jakarta Sans', sans-serif !important;
            color: #ff6666 !important;       /* Lighter red color */
            font-size: 1.1rem !important;    /* Slightly bigger font */
    ...
    <script>
        $(document).ready(function() {{
            ...
            $('.works-table').DataTable({{
                responsive: true,
                pageLength: 50,
                order: [[2, 'asc']], // Sort by published date by default
                columnDefs: [{{
...
<head>
    ...
    <style>
        :root {{
            ...
            --surface-2: #262a2f;
            --primary: #bb0a1e;
            --primary-light: #d41d33;
            --primary-dark: #8b0816;
            --accent: #ff1f3d;
            --text: #ffffff;
            --text-secondary: rgba(255, 255, 255, 0.7);
            --border: rgba(255, 255, 255, 0.12);
            /* Adding blood red colors */
            --blood-red: #8B0000;
            --blood-red-light: #A30000;
            --blood-red-hover: #CC0000;
        }}
...
<body>
    ...
    def fix_missing_dates(self, works_list):
        ...
        for work in works_list:
            if work.work_type.lower() in ['collection', 'anthology', 'story collection']:
                if work.published_date and work.published_date != '0000-00-00':
                    print(f"Found collection: {work.title} with date {work.published_date}")  # Debug print
                    collection_dates[work.title] = work.published_date
        ...
        for work in works_list:
            if work.published_date == '0000-00-00' and work.available_in:
                ...
                if '=HYPERLINK' in collection_title:
                    match = re.search(r'=HYPERLINK\("[^"]*",\s*"([^"]+)"\)', collection_title)
                    if match:
                ...
                if collection_title in collection_dates:
                    ...
                    print(f"Updated {work.title} with date {work.published_date} from collection {collection_title}")  # Debug print
