...
class KingWorksParser:
    ...
    def sync_collection_dates(self, works_list):
        ...
        for work in works_list:
            logger.debug(f"Checking work: {work.title} (Type: {work.work_type}, Date: {work.published_date})")
            if work.work_type.lower() in ['collection', 'anthology', 'story collection']:
                if work.published_date and work.published_date != '0000-00-00':
                    logger.info(f"Found collection: {work.title} with date: {work.published_date}")
                    collection_dates[work.title] = work.published_date
                    collection_dates[self.clean_title(work.title)] = work.published_date

        logger.info(f"Found {len(collection_dates)} collections with valid dates:")
        for coll, date in collection_dates.items():
            logger.info(f"  - {coll}: {date}")
        ...
        for work in works_list:
            if (not work.published_date or work.published_date == '0000-00-00') and work.available_in:
                ...
                logger.info(f"  Current date: {work.published_date}")
                logger.info(f"  Available in: {work.available_in}")
                ...
                if collection_name in collection_dates:
                    ...
                    updated_count += 1
                    logger.info(f"  Updated date (direct match): {work.published_date}")
                elif cleaned_collection_name in collection_dates:
                    work.published_date = collection_dates[cleaned_collection_name]
                    updated_count += 1
                    logger.info(f"  Updated date (cleaned match): {work.published_date}")
                else:
                    logger.info(f"  No matching collection found for: {collection_name}")
                    logger.info(f"  Cleaned name was: {cleaned_collection_name}")

        logger.info(f"\nFinished synchronization. Updated {updated_count} works.")
    ...
    def generate_html_table(self, rows: List[List[str]]) -> str:
        ...
        headers = ['Read', 'Owned', 'Published', 'Title', 'Type', 'Collection',
                  'Hardcover', 'Paperback', 'Ebook', 'Audiobook', 'Movie', 'Miniseries']
        ...
        for row in rows:
            ...
            title = self.extract_title_from_hyperlink(row[3])  # Extract title from the hyperlink in column 3
            html.append(f'<td><input type="checkbox" class="status-checkbox" data-type="read" data-title="{title}"></td>')
            # Handle Owned checkbox (column 1)
            html.append(f'<td><input type="checkbox" class="status-checkbox" data-type="owned" data-title="{title}"></td>')
    ...
    <script>
        $(document).ready(function() {{
            $('.works-table').DataTable({{
                responsive: true,
                pageLength: 25,
                order: [[2, 'asc']], // Sort by Published date (column index 2) by default
                columnDefs: [
                    {{
                        targets: 2, // Published date column
                        type: 'date'
                    ...
                    {{
                        targets: [0, 1], // Read and Owned columns
                        orderable: false
                    }}
                ],
                language: {{
