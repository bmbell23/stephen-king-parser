...
class KingWorksParser:
    ...
    def sync_collection_dates(self):
        ...
        for title, row_data in self.works_dict.items():
            ...
            current_date = row_data[1]

            # Check if work is in a collection and has no valid date
            if available_in_hyperlink and (
                current_date == "0000-00-00" or current_date == "9999-99-99"
            ):
    ...
    def format_published_date(self, date_str: str) -> str:
        """Convert empty or '0000-00-00' dates to '9999-99-99' for sorting."""
        if not date_str.strip() or date_str == "0000-00-00":
            return "9999-99-99"
        return date_str
    ...
    def generate_html_table(self, rows: List[List[str]]) -> str:
        ...
        for row in rows:
            ...
            if (
                len(row) < 12
                or str(row[3]).lower() == "title"
                or "a to z" in str(row[3]).lower()
            ):
                continue
            ...
            if not published_date or published_date == "0000-00-00":
                sort_date = "0000-00-00"  # Changed to put blank dates at the end
                display_date = ""
            else:
                try:
    ...
    def parse_and_export(self):
        ...
        for work in works:
            try:
                ...
                failed_count += 1
                print(f"Error processing work data: {str(e)}")
            except AttributeError as e:
                failed_count += 1
                print(f"Error accessing HTML element: {str(e)}")
            except KeyError as e:
                failed_count += 1
                print(f"Error accessing required field: {str(e)}")

        print(
            f"Successfully processed {processed_count} works, "
            f"failed to process {failed_count} works"
        )
