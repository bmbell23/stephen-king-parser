...
class KingWorksParser:
    ...
    def sync_collection_dates(self, works_list):
        ...
        for work in works_list:
            logger.debug(
                f"Checking work: {work.title} (Type: {work.work_type}, Date: {work.published_date})"
            )
        ...
        for work in works_list:
            if (
                ...
                if collection_name in collection_dates:
                    ...
                    updated_count += 1
                    logger.info(
                        f"  Updated date (cleaned match): {work.published_date}"
                    )
                else:
                    logger.info(
                        f"  No matching collection found for: {collection_name}"
                    )
